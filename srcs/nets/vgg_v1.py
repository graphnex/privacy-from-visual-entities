#!/usr/bin/env python
#
# Code taken from https://github.com/guang-yanng/Image_Privacy/blob/master/networks/vgg_v1.py
#
##################################################################################
import math
...
def vgg16_rois_v1(pretrained=True):
    ...
    if pretrained:
        # model_dir = '/home/yangguang/.torch/models/'
        # pretrained_model = torch.load(model_dir + 'vgg16-397923af.pth')
...
class VGG16_ROIS_v1(nn.Module):
    def __init__(self, features, init_weights=True):
        ...
        print("VGG-16 features - Trainable parameters")
        print_model_parameters(self.features)

        self.classifier = nn.Sequential(
            nn.Linear(512 * 7 * 7, 4096),  # 102,764,544 trainable parameters
            nn.ReLU(True),
            nn.Dropout(),
            nn.Linear(4096, 4096),  # 16,781,312 trainable parameters
        )
        print("VGG-16 classifier - Trainable parameters")
    ...
    def _crop_pool_layer(self, bottom, rois, categories, max_pool=False):
        ...
        # Added second dimension (n rois) that was missing
        x1 = rois[:, 0::4] / 16.0
        y1 = rois[:, 1::4] / 16.0
        x2 = rois[:, 2::4] / 16.0
        y2 = rois[:, 3::4] / 16.0
        ...
        for b in range(bottom.size(0)):
            if categories[b, 0].item() == 0:
                continue

            flag = False

            end_idx += int(categories[b, 0].item())
            ...
            if b == 0 or "crops_all" not in dir():
                crops_all = crops
            else:
                crops_all = torch.cat((crops_all, crops), 0)
            start_idx = end_idx

        # if 'crops_all' not in dir():
        #     crops_all = torch.zeros(0, 512, 7, 7)

        if flag:
            crops_all = torch.zeros(bottom.size(0), 512, 7, 7).cuda()

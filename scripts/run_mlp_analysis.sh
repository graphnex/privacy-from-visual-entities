...
for R in {1..6}
do
    CONFIG=$ROOT_DIR/configs/analysis_mlp/${MODEL_NAME}_v${R}.json
...
# Backup model and results
    ...
    zip -ur $DSTDIR/$myzipfile configs/analysis_mlp/${MODEL_NAME}_v${R}.json
    ...
    echo "MLP model and results backed up! (run ${R})"
done
